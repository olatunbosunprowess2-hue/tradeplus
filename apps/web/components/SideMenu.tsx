...
export default function SideMenu({ className = '' }: SideMenuProps) {
    ...
    const menuContent = (
        <>
            ...
            {isOpen && (
                <div
                    className="fixed inset-0 bg-black/50 z-[10050] transition-opacity"
            ...
            {/* Side Drawer */}
            <div
                className={`fixed top-0 right-0 bottom-0 w-64 max-w-[65vw] bg-white shadow-2xl z-[10060] transform transition-transform duration-300 ease-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'
                    }`}
            >
                {/* Header */}
        ...
        <>
            ...
            >
                <div className="relative hover:bg-gray-100 p-1.5 rounded-full transition-colors group">
                    ...
                    {/* Online status dot if authenticated */}
                    {isAuthenticated && user && (
                        <div className="absolute bottom-1 right-1 w-2.5 h-2.5 bg-green-500 border-2 border-white rounded-full z-20 shadow-sm" />
                    )}
                </div>
            </button>
