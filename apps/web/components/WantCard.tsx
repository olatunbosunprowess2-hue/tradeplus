...
import { WantItem, useWantsStore } from '@/lib/wants-store';

interface WantCardProps {
    item: WantItem;
}

export default function WantCard({ item }: WantCardProps) {
    const { toggleFulfilled, removeItem } = useWantsStore();
        ...
        >
            ...
            <div className="mt-auto pt-3 border-t border-black/5 flex items-center">
                <label className="flex items-center gap-2 cursor-pointer group">
                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${item.isFulfilled ? 'bg-green-500 border-green-500' : 'bg-white border-gray-300 group-hover:border-green-500'
                        }`}>
                        {item.isFulfilled && (
                    ...
                    <span className={`text-sm font-medium transition-colors ${item.isFulfilled ? 'text-green-600' : 'text-gray-500 group-hover:text-gray-700'
                        }`}>
                        {item.isFulfilled ? 'Fulfilled' : 'Mark as Done'}
                    </span>
                    <input
                        type="checkbox"
                        className="hidden"
                        checked={item.isFulfilled}
                        onChange={() => toggleFulfilled(item.id)}
                    />
                </label>
            </div>
        </div>
