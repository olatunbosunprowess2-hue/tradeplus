...
export default function ChatPage() {
    ...
    const handleFirstChatAgree = async () => {
        setShowFirstChatModal(false);
        if (conversationId) {
            localStorage.setItem(`first_chat_conv_${conversationId}`, 'true');
        }

        // If there's an active accepted barter offer and timer is not started yet, trigger start-timer API
        const offer = conversation?.barterOffer;
        if (offer && offer.status === 'accepted' && !offer.timerExpiresAt) {
            try {
                await apiClient.patch(`/barter/offers/${offer.id}/start-timer`);
                toast.success('Trade timer started! You have 60 minutes to coordinate.');
                // Refresh conversations to pull latest timer state
                await fetchConversations();
            } catch (err: any) {
                console.error('Failed to start trade timer:', err);
            }
        }
    };
    ...
    useEffect(() => {
        if (conversationId && !conversationId.startsWith('temp-')) {
            ...
            const hasShownModal = localStorage.getItem(`first_chat_conv_${conversationId}`);
            if (!hasShownModal) {
                setShowFirstChatModal(true);
            }
        }
    }, [conversationId, fetchMessages, markAsRead]); // Stabilized dependencies
                        ...
                        {showMenu && (
                            <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-lg py-1.5 z-50 animate-in fade-in slide-in-from-top-1">
                                <button
                                    ...
                                    className="w-full text-left px-4 py-2.5 text-xs font-bold text-gray-700 hover:bg-gray-50 flex items-center gap-2 transition-colors duration-200 cursor-pointer"
                                >
                                    <svg className="w-3.5 h-3.5 text-amber-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                                ...
                                {conversation.barterOffer && ['accepted', 'awaiting_fulfillment', 'awaiting_meetup'].includes(conversation.barterOffer.status) && (
                                    <button
                                        ...
                                        className="w-full text-left px-4 py-2.5 text-xs font-bold text-red-600 hover:bg-red-50 flex items-center gap-2 border-t border-gray-50 transition-colors duration-200 cursor-pointer"
                                    >
                                        <svg className="w-3.5 h-3.5 text-red-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M20.618 5.984A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016zM12 9v2m0 4h.01" /></svg>
                                        <span>Dispute Trade</span>
                                    </button>
                                )}
                            </div>
                        )}
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-xs font-semibold text-blue-700 mb-0.5 flex items-center gap-1">
                                    <svg className="w-3.5 h-3.5 text-blue-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
            ...
            <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-4 py-4">
                <div className="container mx-auto max-w-4xl space-y-3">
                    ...
                    {conversation.barterOffer && 
                      ['accepted', 'awaiting_fulfillment', 'awaiting_meetup'].includes(conversation.barterOffer.status) && 
                      conversation.barterOffer.timerExpiresAt && 
                      !isOnline(conversation.participantLastActiveAt) && (
                         <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-3 shadow-sm animate-in fade-in slide-in-from-top-2 duration-300">
                             <div className="w-10 h-10 bg-amber-100 rounded-xl flex items-center justify-center shrink-0 text-lg">
                                 🔔
                             </div>
                             <div className="flex-1">
                                 <h4 className="text-sm font-bold text-amber-900">Partner is currently offline</h4>
                                 <p className="text-xs text-amber-700 mt-0.5 leading-relaxed font-semibold">
                                     {conversation.participantName || 'Your trade partner'} is offline right now, but they have been notified via email to coordinate the exchange. Messages you send here will trigger notification pings to their email.
                                 </p>
                             </div>
                         </div>
                     )}
                    ...
                    {messages.length === 0 && !hasFetchedMessages ? (
                        ...
                        <div className="text-center py-12">
                             <div className="w-16 h-16 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center mx-auto mb-4 shadow-inner">
                                 <svg className="w-8 h-8 text-blue-600 animate-[bounce_1.5s_infinite]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 11.5V14m0-2.5v-6a1.5 1.5 0 113 0m-3 6a1.5 1.5 0 00-3 0v2a7.5 7.5 0 0015 0v-5a1.5 1.5 0 00-3 0m-6-3V11m0-5.5a1.5 1.5 0 013 0v5" /></svg>
                             </div>
